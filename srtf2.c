...
void WaitingTime(int n, int BT[], int REM_BT[], int AT[], int WT[], int PID[])
{
    int t, count = 0;
    REM_BT[50] = 100;
    ...
    for (t = 0 ; count != n ; t++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            else if(AT[i] <= t && AT[i] < AT[m] && REM_BT[i] == REM_BT[m] && REM_BT[i] > 0){
                m=i;
            }
            else if(AT[i] <= t && AT[i] == AT[m] && REM_BT[i] == REM_BT[m] && PID[i] < PID[m] && REM_BT[i] > 0)
                {
                m=i;
            }
            else 0;
...
main()
{
    ...
    WaitingTime(n , BT , REM_BT , AT , WT, PID);
